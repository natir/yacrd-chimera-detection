...
rule all:
    input:
        ...
        [f"yacrd/{reads_name}_cov{cov}_i{i}_c{c}_l{length}.g{config['g_distance'][reads_name]}.yacrd"
...
rule reads2reads_gc:
    input:
        reads = "reads/{prefix}_{techno}.fasta"
    output:
        ovl = "scrubbing/{prefix}_{techno}.g{dist}.paf"
    shell:
...
rule yacrd_gc:
    input:
        ovl = "overlaps/{reads_name}_i{i}_c{c}_l{length}.g{dist}.paf"
    output:
        chimeric_report = "yacrd/{reads_name}_cov{cov}_i{i}_c{c}_l{length}.g{dist}.yacrd"
    shell:
        "yacrd chimeric -c {wildcards.cov} -i {input.ovl} -o {output.chimeric_report}"
